...
class LabelsFragment : Fragment(), LabelsAdapter.LabelItemListener {
    ...
    override fun onLabelClicked(labelItem: LabelItem) {
        if (labelItem.isSpecial) {
            // For "ALL" label, disable filtering and navigate back to the main screen
            sharedViewModel.toggleActiveFiltering(false)
    ...
    override fun onLabelActiveChanged(labelItem: LabelItem, isActive: Boolean) {
        if (labelItem.isSpecial) {
            ...
            // Enable/disable filtering based on ALL checkbox
            sharedViewModel.toggleActiveFiltering(isActive)
        } else {
